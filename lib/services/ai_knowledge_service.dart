...
class AIKnowledgeService {
  static const String _knowledgeBaseVersion = "2.0.0";
  
  /// Base de conhecimento sobre o Challenge Vision e ChallengeMind
  static const Map<String, dynamic> _challengeVisionKnowledge = {
    "application": {
      "name": "Challenge Vision",
      "ai_assistant_name": "ChallengeMind",
      "version": "2.0.0",
      "description": "Aplicativo de gestão de projetos farmacêuticos com IA integrada",
      "purpose": "Centralizar e otimizar a gestão de projetos de inovação, pesquisa, desenvolvimento e melhoria na Eurofarma",
      "target_users": [
        "Gestores de projeto",
        "Líderes de equipe",
        "Analistas de P&D",
    ...
    "challenge_mind": {
      "name": "ChallengeMind",
      "role": "Assistente de IA especializado em gestão de projetos farmacêuticos",
      "personality": "Profissional, analítico, proativo e sempre focado em resultados",
      "capabilities": [
      ...
      "specializations": [
        "Projetos de inovação farmacêutica",
        "Gestão de P&D",
        "Análise de dados de projetos",
        "Otimização de processos",
        "Gestão de riscos regulatórios",
        "Estratégias de time-to-market"
      ],
      "communication_style": {
        "tone": "Profissional e acolhedor",
        "approach": "Baseado em dados e evidências",
        "focus": "Soluções práticas e acionáveis",
        "language": "Técnico quando necessário, acessível sempre"
      }
    },
    "app_architecture": {
      "frontend": "Flutter (Multiplataforma)",
      "backend": "Firebase (Firestore, Auth, Analytics)",
      "local_storage": "Hive (Offline-first)",
      "ai_integration": "OpenAI GPT-4 / Claude",
      "sync_strategy": "Real-time com fallback offline",
      "security": "Autenticação Firebase + criptografia local"
    },
    "project_workflow": {
      "creation": "Formulário guiado com validação inteligente",
      "categorization": "Automática baseada em descrição e contexto",
      "tracking": "Status em tempo real com notificações",
      "analytics": "Dashboard com métricas visuais",
      "collaboration": "Compartilhamento e comentários",
      "archiving": "Histórico completo com lições aprendidas"
    },
    "ai_features": {
      "smart_categorization": "Classifica projetos automaticamente",
      "risk_assessment": "Identifica riscos baseado em padrões",
      "timeline_optimization": "Sugere cronogramas realistas",
      "resource_allocation": "Recomenda distribuição de recursos",
      "performance_prediction": "Prevê sucesso baseado em histórico",
      "insight_generation": "Gera insights automáticos",
      "conversation_memory": "Lembra contexto de conversas anteriores"
    }
  };
  ...
  static String generateEnrichedContext(List<Project> projects) {
    ...
    // Informações sobre o ChallengeVision e ChallengeMind
    context.writeln("=== SOBRE O CHALLENGEVISION E CHALLENGEMIND ===");
    context.writeln("Aplicativo: ${_challengeVisionKnowledge['application']['name']} v${_challengeVisionKnowledge['application']['version']}");
    context.writeln("IA Assistente: ${_challengeVisionKnowledge['challenge_mind']['name']}");
    context.writeln("Descrição: ${_challengeVisionKnowledge['application']['description']}");
    context.writeln("Propósito: ${_challengeVisionKnowledge['application']['purpose']}");
    context.writeln();
    
    context.writeln("=== CHALLENGEMIND - SUA IDENTIDADE ===");
    context.writeln("Nome: ${_challengeVisionKnowledge['challenge_mind']['name']}");
    context.writeln("Papel: ${_challengeVisionKnowledge['challenge_mind']['role']}");
    context.writeln("Personalidade: ${_challengeVisionKnowledge['challenge_mind']['personality']}");
    context.writeln("Capacidades principais:");
    for (final capability in _challengeVisionKnowledge['challenge_mind']['capabilities']) {
      context.writeln("- $capability");
    }
    context.writeln();
    
    context.writeln("Especializações:");
    for (final specialization in _challengeVisionKnowledge['challenge_mind']['specializations']) {
      context.writeln("- $specialization");
    }
    context.writeln();
    
    context.writeln("Estilo de comunicação:");
    final commStyle = _challengeVisionKnowledge['challenge_mind']['communication_style'];
    context.writeln("- Tom: ${commStyle['tone']}");
    context.writeln("- Abordagem: ${commStyle['approach']}");
    context.writeln("- Foco: ${commStyle['focus']}");
    context.writeln("- Linguagem: ${commStyle['language']}");
    context.writeln();
    
    context.writeln("=== ARQUITETURA DO APLICATIVO ===");
    final architecture = _challengeVisionKnowledge['app_architecture'];
    context.writeln("Frontend: ${architecture['frontend']}");
    context.writeln("Backend: ${architecture['backend']}");
    context.writeln("Armazenamento local: ${architecture['local_storage']}");
    context.writeln("Integração IA: ${architecture['ai_integration']}");
    context.writeln("Estratégia de sincronização: ${architecture['sync_strategy']}");
    context.writeln("Segurança: ${architecture['security']}");
    context.writeln();
    
    context.writeln("=== RECURSOS DE IA DISPONÍVEIS ===");
    final aiFeatures = _challengeVisionKnowledge['ai_features'];
    for (final feature in aiFeatures.keys) {
      // Formatação limpa sem asteriscos ou backticks
      final cleanFeature = feature.replaceAll('_', ' ').toUpperCase();
      context.writeln("• $cleanFeature: ${aiFeatures[feature]}");
    }
    context.writeln();
...
""";
  ...
  static String _generateGeneralPrompt(List<Project> projects) {
    return """
Você é o ChallengeMind, assistente de IA especializado em gestão de projetos farmacêuticos da Eurofarma.

=== SUA IDENTIDADE ===
Nome: ChallengeMind
Papel: Assistente de IA especializado em gestão de projetos farmacêuticos
Personalidade: Profissional, analítico, proativo e sempre focado em resultados
Tom: Profissional e acolhedor
Abordagem: Baseado em dados e evidências
Foco: Soluções práticas e acionáveis
Linguagem: Técnico quando necessário, acessível sempre
...
6. Analisar performance e tendências
7. Recomendar estratégias de otimização
...
Use o contexto fornecido sobre os projetos, a arquitetura do ChallengeVision e as regras de negócio da empresa.

Lembre-se: Você é parte integrante do ChallengeVision, um aplicativo de gestão de projetos farmacêuticos com IA integrada.
""";
  }
  ...
  /// Obtém informações sobre o ChallengeVision
  static Map<String, dynamic>? getChallengeVisionInfo() {
    return _challengeVisionKnowledge['application'];
  }
  
  /// Obtém informações sobre o ChallengeMind
  static Map<String, dynamic>? getChallengeMindInfo() {
    return _challengeVisionKnowledge['challenge_mind'];
  }
  
  /// Obtém informações sobre a arquitetura do app
  static Map<String, dynamic>? getAppArchitectureInfo() {
    return _challengeVisionKnowledge['app_architecture'];
  }
  
  /// Obtém informações sobre os recursos de IA
  static Map<String, dynamic>? getAIFeaturesInfo() {
    return _challengeVisionKnowledge['ai_features'];
  }
  
  /// Gera prompt de apresentação do ChallengeMind
  static String generateChallengeMindIntroduction() {
    final mindInfo = _challengeVisionKnowledge['challenge_mind'];
    final appInfo = _challengeVisionKnowledge['application'];
    
    return """
Olá! Eu sou o ${mindInfo['name']}, seu assistente de IA especializado em gestão de projetos farmacêuticos.

Sou parte integrante do ${appInfo['name']}, um aplicativo desenvolvido para centralizar e otimizar a gestão de projetos de inovação, pesquisa, desenvolvimento e melhoria na Eurofarma.

COMO POSSO AJUDAR:
• Analisar riscos e oportunidades nos seus projetos
• Sugerir otimizações e melhorias
• Identificar gargalos e problemas potenciais
• Fornecer insights baseados em dados
• Recomendar estratégias de gestão
...
Estou aqui para apoiar você com análises precisas, recomendações práticas e insights estratégicos. Como posso ajudar com seus projetos hoje?
""";
  }
}
