...
class ProjectStorageService {
  ...
  Future<void> syncFromCloud() async {
    try {
      ...
      final currentUser = FirebaseAuth.instance.currentUser;
      if (currentUser == null) {
        print('ℹ️ SYNC: Usuário não logado, pulando sincronização');
        return;
      }

      print('🔄 SYNC: Iniciando sincronização da nuvem...');
      print('🔄 SYNC: Usuário atual: ${currentUser.email}');
      print('🔄 SYNC: UID do usuário: ${currentUser.uid}');
      ...
      if (cloudProjects.isNotEmpty) {
        ...
        print('✅ SYNC: ${cloudProjects.length} projetos sincronizados da nuvem');
      } else {
        print('ℹ️ SYNC: Nenhum projeto encontrado na nuvem');
      }
    } catch (e) {
      // Tratamento silencioso para erros de permissão do Firebase
      if (e.toString().contains('permission-denied')) {
        print('ℹ️ SYNC: Firebase não configurado, continuando offline');
        return; // Não propagar o erro
      } else if (e.toString().contains('network')) {
        print('ℹ️ SYNC: Sem conexão com a internet, continuando offline');
        return; // Não propagar o erro
      } else {
        print('⚠️ SYNC: Erro na sincronização: $e');
        print('ℹ️ SYNC: Continuando em modo offline');
        return; // Não propagar o erro
      }
    }
  }
