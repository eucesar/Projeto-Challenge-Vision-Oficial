...
import 'package:challenge_vision/services/gemini_api_service.dart';
...
class _ChallengeMindScreenState extends State<ChallengeMindScreen> {
  ...
  /// Limpa formatação problemática das respostas da IA
  ...
  Future<void> _sendMessage() async {
    ...
    try {
      ...
      final additionalContext = {
        ...
        'projects': widget.projects.map((p) => {
          'name': p.name,
          'category': p.category,
          'status': p.status,
      ...
      // Usar o novo serviço do Gemini
      final geminiResponse = await GeminiApiService.sendMessage(
        message: userMessage,
      ...
      if (geminiResponse.success) {
        setState(() {
          _messages.add({"role": "bot", "msg": geminiResponse.message});
        });
        ...
        // Tratar erro
        String errorMessage = "❌ Erro na API: ${geminiResponse.message}";
        if (geminiResponse.error != null) {
          errorMessage += "\n\nDetalhes: ${geminiResponse.error}";
        }
      ...
      Navigator.of(context).push(
        MaterialPageRoute(
          builder: (context) => ProjectCustomizationScreen(
            ...
            onNavigateBack: () {
              // Resetar estado quando voltar para ChallengeMind
              WidgetsBinding.instance.addPostFrameCallback((_) {
                if (mounted) {
                  setState(() {
                    _selectedIndex = 1; // Voltar para ChallengeMind
                  });
                }
              });
            },
