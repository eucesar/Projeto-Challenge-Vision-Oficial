...
import 'package:challenge_vision/widgets/animated_screen_wrapper.dart'; // Importar animações
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void didChangeDependencies() {
    ...
    // Resetar seleção para Home quando a tela for exibida
  ...
  Future<void> _loadProjects() async {
    ...
    // Tentar sincronizar do Firebase (silenciosamente)
    try {
      await _storageService.syncFromCloud();
    } catch (e) {
      // Erro silencioso - continuar offline
    }
    
    // Carregar do Hive (sempre funciona)
  ...
  void _onItemTapped(int index) {
    ...
    if (index == 0) {
      // Home - já estamos aqui, garantir que está selecionado
      setState(() {
        _selectedIndex = 0;
      });
      return;
  ...
  void _navigateToChallengeBot() {
    Navigator.of(context).push(
      PageTransition(
        child: ChallengeMindScreen(
          projects: _projects,
          fromTabIndex: _lastActiveIndex, // Passar índice de origem
          onNavigateBack: () {
            // Resetar estado quando voltar para Home
            WidgetsBinding.instance.addPostFrameCallback((_) {
              if (mounted) {
                setState(() {
                  _selectedIndex = 0; // Sempre voltar para Home quando retornar
                  _lastActiveIndex = 0;
                });
              }
            });
  ...
  void _navigateToAddProject() {
    Navigator.of(context).push(
      PageTransition(
        child: AddProjectScreen(onProjectCreated: _addNewProject),
      ),
    ).then((_) {
      // Resetar estado quando voltar para Home
      WidgetsBinding.instance.addPostFrameCallback((_) {
  ...
  void _navigateToProjectCustomization() {
    Navigator.of(context).push(
      PageTransition(
        child: ProjectCustomizationScreen(
